...
extension VLContentInfoAnalytics {
    ...
    public convenience init(jsonData: Data) throws {
        ...
        self.init(
            ...
            durationInSeconds: response.duration.map { max(1, Int($0.timeInterval.rounded())) },
            airDate: response.airDate,
            playbackUrl: response.videoUrl,
            streamType: response.duration?.isLive == true ? "live" : "vod",
            videonetwork: response.brand,
            isLive: response.duration?.isLive ?? false,
