...
struct VideoList:Decodable {
    ...
    func checkForConfigurationErrorMessage() -> String?{
        if apiBaseUrl.contains("xxxxx") {
            return "Please set the API base URL and ViewLift token in the config json file."
        }
        else if apiBaseUrl.contains("xxxxx") {
            return "Please set the API base URL in the config json file."
        }else if vlToken.contains("xxxxx") {
            return "Please set the ViewLift token in the config json file."
        }
